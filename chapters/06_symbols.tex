...
\begin{longtable}{@{}p{0.22\textwidth}p{0.72\textwidth}@{}}
    \toprule
    \rowcolor{tablehdr}
    {\color{white}\textbf{Symbol}}
    & {\color{white}\textbf{Meaning (units)}}\\
    \midrule
    \endfirsthead
    \toprule
    \rowcolor{tablehdr}
    ...
    $F$           & Flatness $= S/I$ (dimensionless); peak hammer force
                    (\si{\kilo\newton})\\
    ...
    $T$           & Surface-texture index $= A/\Ach$ (dimensionless)\\
    ...
    $\sigma$      & Nominal contact stress at specimen surface
                    (\si{\kilo\pascal})\\
    $\Delta X$    & Change in descriptor
                    $X = X_{\text{after}} - X_{\text{before}}$\\
    $\%\Delta X$  & Percentage change in
                    $X = \Delta X / X_{\text{before}} \times 100$\\
    $\Psi$        & Sphericity
                    $= \pi^{1/3}\,(6V)^{2/3}/A$
                    \citep{Wadell1932}\\
    $\kappa_{s}$  & Sphericity ratio $= \Deq/L$ (dimensionless)\\
\end{longtable}
