...
mat product (Getzner Werkstoffe GmbH, Bürs, Austria). Getzner is a widely
adopted supplier of polyurethane track-resilient elements on European,
Australian, and Indian mainline and urban rail networks. The specific
variant used in this study has a nominal thickness of
\SI{15}{\milli\metre}, a Shore~A hardness of approximately~45, a bulk
density of approximately \SI{0.45}{\gram\per\centi\metre\cubed}, and a
nominal static bedding modulus of
\SI{0.06}{\newton\per\milli\metre\cubed}---placing it in the softer half
of the \SIrange{0.03}{0.12}{\newton\per\milli\metre\cubed} range in which
Getzner \gls{UBM} products are typically deployed beneath ballast in
transition zones and impact-prone locations. The \gls{PUBM} disc was cut
to \SI{300}{\milli\metre} diameter matching the mould internal diameter,
like the \gls{RUBM}. Compared with the \gls{RUBM}
(Shore~A~$\approx$~65, \SI{12}{\milli\metre} thick,
\SI{1.35}{\gram\per\centi\metre\cubed}), the \gls{PUBM} is thicker,
softer, and lighter---a combination that provides a larger elastic
deformation range per blow and potentially greater impulse-energy
absorption at higher strain rates.

\citet{Gundavaram2023} have previously demonstrated, in cyclic-triaxial
tests on Indian ballast, that an elastomeric polyurethane inclusion
materially reduces both cumulative plastic deformation and the rate of
fines generation, and the use of \gls{PUBM} in the present programme
extends that line of evidence from cyclic to impact loading. Like the
\gls{RUBM}, a fresh \gls{PUBM} disc was cut for every test to eliminate
carry-over effects from cumulative pad damage.
