...
This chapter documents the materials, the apparatus, the test protocol, and
...
bulk-index computation (Section~\ref{sec:meth-protocol}), the image-based
three-dimensional morphological-analysis pipeline
(Section~\ref{sec:meth-morphology}), and the statistical framework that
supports the analysis of Chapters~\ref{chap:impact-results}
and~\ref{chap:morph-results}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.92\textwidth]{image9.png}
    \caption{Experimental methodology flowchart (10-stage programme).}
...
quarry supplying RDSO-specification railway ballast in the Vadodara region
of Gujarat, India. A single source was used to eliminate parent-rock
...
(\(<\SI{1}{\percent}\)). These figures are consistent with the RDSO
specification (RDSO, 2023) requirement of not less than
...
(Koohmishi \& Palassi, 2020; Zhang et~al., 2023).
...
the RDSO procedure (RDSO, 2023). The resulting gradation, plotted in
Figure~\ref{fig:meth-psd} against the RDSO upper and lower envelope curves,
shows that the ballast conforms to the specification with a small reserve
against both limits---the target gradation sits close to the mid-band of the
envelope throughout the \SIrange{20}{63}{\milli\metre} range. The percentage
retained on the \SI{65}{\milli\metre} sieve was below the \SI{5}{\percent}
maximum permitted by the specification; the percentage passing the
\SI{20}{\milli\metre} sieve was below \SI{2}{\percent}, well within the
\SI{2}{\percent} maximum fines limit. The gradation is therefore centred
within the acceptable window and is representative of production-quality
RDSO ballast.
...
\begin{figure}[ht]
    ...
    \caption{Pre-test particle size distribution of the basalt ballast used
             ...
             lower envelope curves of the RDSO specification.}
    \label{fig:meth-psd}
\end{figure}
...
required by the RDSO specification. The measurements are summarised in
Table~\ref{tab:meth-ballast-properties}. All values fall within the RDSO
acceptance thresholds; the Aggregate Impact Value measured on the
artificially crushed \SIrange{10}{12.5}{\milli\metre} sub-sample is
reported here for completeness but is precisely the index whose limitations
are set out in Chapters~\ref{chap:introduction} and~\ref{chap:literature}.
The remainder of the programme accordingly reports the bulk fouling index
\FIIN{} on the full-size specimen.
...
\begin{tabular}{@{}lccc@{}}
    \toprule
    \textbf{Property} & \textbf{Measured} & \textbf{RDSO limit}
        & \textbf{Method}\\
    \midrule
    ...
    Aggregate Abrasion Value (LAA) (\si{\percent})
        & 15.4 & \(\le 30\) & IS:2386 Pt~IV\\
    Aggregate Impact Value (\si{\percent})
...
\begin{figure}[ht]
    ...
    \caption{Pile of the RDSO-gradation basalt ballast used in the
             experimental programme (Civil Lab, GSV).}
...
test matrix. The three pad conditions are an Unreinforced (UR) control, a
Rubber Under-Ballast Mat (RUBM), and a Polyurethane Under-Ballast Mat
(PUBM). The two base conditions are a Flexible Base (FB) of compacted sand
and a Rigid Base (RB) of the concrete mould floor. The systematic
three-letter abbreviations are introduced formally in this thesis as a
replacement for the ad-hoc numbered test labels used in earlier laboratory
work.

\subsection{Unreinforced (UR) control}
\label{sec:meth-UR}

The UR configuration places no pad between the ballast fill and the base
element of the mould. The UR condition serves three analytical purposes
simultaneously: it is the mechanical control against which the two pad
conditions are compared; it represents the in-service condition of legacy
ballasted track with no substructure reinforcement; and it isolates the
effect of the base condition (flexible sand versus rigid concrete) from the
effect of any compliant pad placed above it. The UR condition is present in
three of the nine test configurations, providing the baseline against which
the pad-protected configurations are measured.

\subsection{Rubber Under-Ballast Mat (RUBM)}
\label{sec:meth-RUBM}

The RUBM material is a commercially manufactured \SI{12}{\milli\metre}
...
the RUBM falls in the middle of the range reported in the literature for
natural-rubber under-ballast mats (Navaratnarajah \& Indraratna, 2017; Ngo
et~al., 2019)---sufficient to dissipate a meaningful fraction of the
hammer impulse energy while stiff enough to recover elastically between
blows without progressive plastic deformation of the pad itself. No RUBM
specimen was reused between tests; a fresh \SI{12}{\milli\metre} disc was
cut for every test to eliminate any carry-over effect of cumulative pad
damage.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.55\textwidth]{image12.jpeg}
    \caption{Rubber Under-Ballast Mat (RUBM) disc, \SI{12}{\milli\metre}
             thick natural rubber, cut to \SI{300}{\milli\metre} diameter.}
    \label{fig:meth-RUBM}
\end{figure}
...
The PUBM material is a commercial Getzner polyurethane under-ballast mat
product. Getzner is a widely adopted supplier of polyurethane
...
locations. The PUBM disc was cut to \SI{300}{\milli\metre} diameter matching
the mould internal diameter, like the RUBM. Gundavaram and Hussaini (2023)
...
use of PUBM in the present programme extends that line of evidence from
cyclic to impact loading.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.55\textwidth]{image13.jpeg}
    \caption{Polyurethane Under-Ballast Mat (PUBM, commercial Getzner
             product), cut to \SI{300}{\milli\metre} diameter.}
    \label{fig:meth-PUBM}
\end{figure}
...
experimental design. The FB condition comprises a \SI{100}{\milli\metre}
deep cushion of clean, medium-grained quartz sand placed at the base of the
test mould before the pad and ballast fill. The sand was oven-dried to
constant mass and compacted in the mould by a standardised vibratory
compaction protocol to a bulk density of approximately
\SI{1.60}{\gram\per\centi\metre\cubed}. The RB condition places the ballast
fill (with or without the interposed pad) directly on the concrete floor of
the test mould. The concrete floor is several orders of magnitude stiffer
than either the pad or the sand base and is effectively unyielding at the
stress levels delivered by the hammer.

The two base conditions are chosen to bracket the stiffness range that a
ballast layer might encounter in service: the FB approximates a
well-compacted granular sub-ballast, while the RB approximates an
...
separated cleanly, which is the analytical design that Nimbalkar,
Indraratna, Dash, and Christie (2012) identified as essential for
distinguishing pad performance from sub-grade compliance in laboratory
shock-mat work.
...
\begin{figure}[ht]
    ...
    \caption{Test mould prepared with the \SI{100}{\milli\metre} flexible
             base sand cushion (FB condition), photographed immediately
             before placement of the pad and the ballast fill.}
    \label{fig:meth-FB}
\end{figure}
...
The thesis rests on a paired-particle experimental design: the same
physical grains are scanned before and after the impact-loading event, and
the grain-scale damage is tracked within each particle rather than averaged
across the population. The paired design is the methodological centre of
gravity of the study, and its value is two-fold. First, it eliminates the
particle-to-particle variability in initial shape---which in any typical
ballast assembly is considerable, with volumes ranging over almost an order
of magnitude across the \SIrange{20}{65}{\milli\metre} gradation---as a
source of scatter in the damage signal. Second, it enables the direct
attribution of each observed change in shape descriptors to the loading
event, because any change recorded in the after-scan that is not also
present in the before-scan of the same particle must have arisen from the
hammer blows.
...
\begin{enumerate}[label=(\roman*),leftmargin=*]
    ...
    \item \textbf{Shape:} particles with extreme flatness or elongation
          ...
          impact loading and would distort the Mode~A / Mode~B
          distribution.
...
Co.\ Ltd., Hangzhou, China) desktop structured-light scanner. The EinScan~SE
was used across the programme, providing comparable geometric accuracy of
approximately \SI{0.1}{\milli\metre} and scan resolution of approximately
\SI{0.15}{\milli\metre} in the automatic turntable mode used here. The
detailed scan-protocol and mesh quality-control procedures are set out in
Section~\ref{sec:meth-morphology} below, in the context of the
...
in STL format into the impact test, and the pre-test mesh serves as the
reference state against which the post-test mesh is paired.
...
\SI{350}{\milli\metre} internal depth, seated on a rigid reaction base-plate;
a vertical guide frame of four mild-steel columns connected by an upper
crosshead; a cylindrical cast-iron hammer of \SI{50}{\kilogram} mass,
sliding on the guide columns in free fall; a continuously adjustable hammer
release-and-locking mechanism graduated over the
...
complete the assembly. The mould diameter of \SI{300}{\milli\metre}
delivers a $D/\dmax$ ratio of approximately 6 against the RDSO
\SI{50}{\milli\metre} mean particle size, eliminating the boundary-dominance
of the conventional IS:2386 Part~IV apparatus (BIS, 1963) and bringing the
present rig into the design envelope established by Nimbalkar, Indraratna,
Dash, and Christie (2012) and the subsequent impact literature surveyed in
Section~\ref{sec:lit-lab}.
...
\begin{figure}[ht]
    ...
    \end{subfigure}
    \caption{Modified drop-weight impact apparatus designed and commissioned
             at GSV. (a) Overall view of the apparatus and its frame.
             (b) Close-up of the load-transfer assembly (bearing plate,
             \SI{5}{\tonne} load cell, striker plate) and the digital
             peak-force display.}
    \label{fig:meth-apparatus}
\end{figure}

\subsection{Comparison with the conventional Aggregate Impact Value
            apparatus}
\label{sec:meth-AIV-comparison}

The salient design differences between the conventional IS:2386 Part~IV
Aggregate Impact Value apparatus and the modified large-scale apparatus
commissioned at GSV are tabulated in Table~\ref{tab:meth-AIV-comparison}.
...
\caption{Principal specification of the modified apparatus in comparison to
         ...
         in the patent documentation (Appendix~\ref{chap:appB}).}
...
\begin{tabular}{@{}p{4.4cm}p{4.8cm}p{5.6cm}@{}}
    \toprule
    \textbf{Parameter} &
    \textbf{Conventional AIV (IS:2386 Pt~IV)} &
    \textbf{Modified apparatus (present)}\\
    \midrule
    Mould internal diameter & \SI{102}{\milli\metre}
        & \SI{300}{\milli\metre}\\
    Mould effective depth & \(\sim\!\SI{50}{\milli\metre}\)
        & \SI{350}{\milli\metre}\\
    Hammer mass & \SI{13.5}{\kilogram} & \SI{50}{\kilogram}\\
    Drop height & \SI{380}{\milli\metre} (fixed)
        & \SIrange{200}{450}{\milli\metre} (adjustable)\\
    Particle size tested
        & \SIrange{10}{12.5}{\milli\metre} (crushed sub-sample)
        & \SIrange{20}{63}{\milli\metre} (full RDSO gradation)\\
    Approx.\ particles per test & \numrange{2}{4} & \numrange{40}{80}\\
    $D/\dmax$ ratio & \(\approx 1.6\) (boundary-dominated) & \(\approx 6\)\\
    Stress simulation & Fixed / none
        & \SIrange{275}{523}{\kilo\pascal} (adjustable, maps to
        \SI{22.5}{\tonne} axle)\\
    Real-time force measurement & None
    ...
    Post-test degradation metric
        & AIV (\(\%\) fines through \SI{2.36}{\milli\metre})
...
\SIrange{500}{600}{\kilo\pascal} range. The in-service stress window at
the top of the ballast layer under Indian Railways traffic is therefore
...
bearing-plate area $\Ap = \pi \times (\SI{0.150}{\metre})^{2} =
\SI{0.0707}{\metre\squared}$, the two apparatus operating points
correspond to:
\begin{align}
    h &= \SI{300}{\milli\metre}: \quad
        \Ke = 50 \times 9.81 \times 0.300 = \SI{147}{\joule}; \quad
        \sigma = \frac{\SI{19.4}{\kilo\newton}}{\SI{0.0707}{\metre\squared}}
            \approx \SI{275}{\kilo\pascal}.
        \label{eq:meth-h300}\\
    h &= \SI{450}{\milli\metre}: \quad
        \Ke = 50 \times 9.81 \times 0.450 = \SI{221}{\joule}; \quad
        \sigma = \frac{\SI{37.0}{\kilo\newton}}{\SI{0.0707}{\metre\squared}}
            \approx \SI{523}{\kilo\pascal}.
        \label{eq:meth-h450}
\end{align}

These two operating points---\SI{275}{\kilo\pascal} at \SI{147}{\joule} and
...
Railways field-loading envelope cleanly. The adjustability of the
drop-height mechanism over the full \SIrange{200}{450}{\milli\metre} range
leaves open the possibility of finer calibration to specific axle-load
...
comparable published drop-weight studies on ballast (Nimbalkar et~al.,
2012; Koohmishi \& Palassi, 2020; Zhang et~al., 2023). Second, preliminary
trials showed that the settlement--blow curve flattens substantially
between blow 15 and blow 20 and enters a near-stable zone beyond blow 15;
twenty blows therefore captures both the transient settlement regime and
the early approach to the steady state.
...
three pad conditions (UR / RUBM / PUBM), the two base conditions (FB / RB),
and the two impact energies (\SI{300}{\milli\metre} and
\SI{450}{\milli\metre} drop heights of the \SI{50}{\kilogram} hammer,
corresponding to nominal contact stresses of \SI{275}{\kilo\pascal} and
\SI{523}{\kilo\pascal} as derived in Section~\ref{sec:meth-calibration}).
The nine cells listed in Table~\ref{tab:meth-matrix} cover the factorial
with three cells (FB--450 combinations) omitted because the apparatus
commissioning revealed that at the \SI{450}{\milli\metre} drop height the
sand cushion under the FB condition would progressively lose bearing
...
\begin{tabular}{@{}cccccccc@{}}
    \toprule
    \textbf{Test No.} & \textbf{Systematic label}
    & \textbf{Pad} & \textbf{Base}
    & $h$ (\si{\milli\metre}) & $\sigma$ (\si{\kilo\pascal})
    & $\Ke$ (\si{\joule})\\
    \midrule
    T1 & UR-FB-300   & UR   & FB & 300 & 275 & 147\\
    T2 & RUBM-FB-300 & RUBM & FB & 300 & 275 & 147\\
    T3 & PUBM-FB-300 & PUBM & FB & 300 & 275 & 147\\
    T4 & UR-RB-450   & UR   & RB & 450 & 523 & 221\\
    T5 & RUBM-RB-450 & RUBM & RB & 450 & 523 & 221\\
    T6 & PUBM-RB-450 & PUBM & RB & 450 & 523 & 221\\
    T7 & UR-RB-300   & UR   & RB & 300 & 275 & 147\\
    T8 & RUBM-RB-300 & RUBM & RB & 300 & 275 & 147\\
    T9 & PUBM-RB-300 & PUBM & RB & 300 & 275 & 147\\
    \bottomrule
\end{tabular}
\end{table}
...
Following the adaptation by Anbazhagan, Lijun, Bharatha, and Pratibha
(2012) of Selig and Waters (1994):
\begin{equation}
    \FIIN = \PoneM + \PtwoM,
    \label{eq:meth-FIIN}
\end{equation}
...
acceptable RDSO ballast gradation.
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
    ...
    \item Export of the validated mesh as a binary STL file.
\end{enumerate}
...
surface-texture index $T=A/\Ach$, sphericity ratio
$\kappa_{s}=\Deq/L$, and the cross-check pair $\nu_{\mathrm{ch}}$ and
$A_{\text{ratio}}$). The classical Wadell sphericity (Wadell, 1932) and
the Krumbein roundness concept (Krumbein, 1941) remain the two principal
reporting axes for grain-scale degradation.

\subsection{Python script}
\label{sec:meth-python}

The descriptors are computed by a purpose-built Python~3 pipeline based on
the \texttt{trimesh} (Dawson-Haggerty, 2019), \texttt{NumPy} (Harris
et~al., 2020), and \texttt{SciPy} (Virtanen et~al., 2020) libraries. The
...
The complete script is reproduced in Appendix~\ref{chap:appB} and has been
